...
static inline
double
kernel(const struct svm_node* x, const struct svm_node* y,
       double gamma, double coef0)
{
  return tanh(gamma*dot(x,y)+coef0);
}

// a gradient of the RBF kernel with respect to gamma
static inline
void
kernel_g(const struct svm_node* x, const struct svm_node* y,
	 double gamma, double coef0, double& gamma_g, double& coef0_g)
{
  double d = dot(x,y);
  double w = cosh(gamma*d+coef0);
  double v = 1.0/(w*w);
  gamma_g = v*d;
  coef0_g = v;
}
		 ...
		 Kmat& kmat, Gmat& gmat)
{
  ...
  for (uint i=0; i!=vec.size(); ++i) {
    for (uint j=i; j!=vec.size(); ++j) {
      k = kernel(&vec[i][0], &vec[j][0], param[0], param[1]);
      kernel_g(&vec[i][0], &vec[j][0], param[0], param[1], g0, g1);
      kmat[i][j] = kmat[j][i] = k;
