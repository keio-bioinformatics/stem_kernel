...
main(int argc, char* argv[])
{
  ...
  if (vm.count("help") || extra_args.empty()) {
    std::cout << "Hyperparameter optimizer for sigmoid kernels" << std::endl
	      << "Usage:" << std::endl
  ...
  Optimizer<SigmoidKernel,GradientComputationAUC> opt;
  opt.optimize(labels, data, fold, param, C, lbd, ubd, nbd,
	       eps, factr, eps, &std::cout);
