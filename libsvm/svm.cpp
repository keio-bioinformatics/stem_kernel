...
#define ROC
		  ...
		  for(j=begin;j<end;j++) {
				target[perm[j]] = svm_predict(submodel,prob->x[perm[j]]);
				int nr_class = svm_get_nr_class(submodel);
				double *dec_values = (double *)malloc(sizeof(double)*nr_class*(nr_class-1)/2);
				svm_predict_values(submodel, prob->x[perm[j]], dec_values);
				fprintf(stdout, "== %d %g ", i, prob->y[perm[j]]);
				for(int ii=0;ii<nr_class;ii++)
				  for(int jj=ii+1;jj<nr_class;jj++) {
				    fprintf(stdout, "%g",
					    *dec_values*submodel->label[0]);
				  }
				fprintf(stdout, "\n");
				free(dec_values);
		  }
